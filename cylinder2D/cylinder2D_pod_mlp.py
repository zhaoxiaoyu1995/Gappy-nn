...
def train():
    ...
    net = MLP(layers=[2, 64, 64, 64, 50]).cuda()

    # Build data loader
    train_dataset = CylinderPodDataset(pod_index=[i for i in range(4250)], index=[i for i in range(3500)],
                                       n_components=50)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = CylinderPodDataset(pod_index=[i for i in range(4250)], index=[i for i in range(3500, 4250)],
                                     n_components=50)
...
def test(index):
    ...
    args.snapshot = '/mnt/jfs/zhaoxiaoyu/Gappy_POD/cylinder2D/logs/ckpt/pod_cylinder_8/best_epoch_299_loss_0.00055745.pth'

    # Define data loader
    test_dataset = CylinderPodDataset(pod_index=[i for i in range(4250)], index=index,
                                      n_components=50)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=4)

    # Load trained network
    net = MLP(layers=[8, 64, 64, 64, 50]).cuda()
