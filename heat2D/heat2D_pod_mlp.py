...
def train():
    ...
    net = MLP(layers=[49, 64, 64, 64, 25]).cuda()

    # Build data loader
    train_dataset = HeatPodDataset(pod_index=[i for i in range(5000)], index=[i for i in range(4000)],
                                   n_components=25)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=4, shuffle=True)
    val_dataset = HeatPodDataset(pod_index=[i for i in range(5000)], index=[i for i in range(4000, 5000)],
                                 n_components=25)
...
def test(index):
    ...
    args.snapshot = '/mnt/jfs/zhaoxiaoyu/Gappy_POD/heat2D/logs/ckpt/pod_heat_16/best_epoch_286_loss_0.00130941.pth'

    # Define data loader
    test_dataset = HeatPodDataset(pod_index=[i for i in range(5000)], index=index,
                                  n_components=25)
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=4)

    # Load trained network
    net = MLP(layers=[16, 64, 64, 64, 25]).cuda()
