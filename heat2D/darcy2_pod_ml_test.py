...
class PODHeat:
    def __init__(self, n_components, positions, pod_index):
        ...
        # Loading data
        f = h5py.File('/mnt/jfs/zhaoxiaoyu/Gappy_POD/fno/darcy1024_train.h5', 'r')
        pca_data = f['u'][pod_index, :, :, :] * 100
        f.close()
    ...
    def prepare_data(self, data_index):
        # Loading data
        f = h5py.File('/mnt/jfs/zhaoxiaoyu/Gappy_POD/fno/darcy1024_train.h5', 'r')
        data_pre = f['pre'][data_index, :, :, :] * 100
        data = f['u'][data_index, :, :, :] * 100
        f.close()
...
if __name__ == '__main__':
    ...
    positions = np.array([[int(84 * np.random.rand(1)), int(84 * np.random.rand(1))] for i in range(300)])
    pod_index = [i for i in range(1000)]
    train_index = [i for i in range(800)]
    val_index = [i for i in range(0, 800)]
    test_index = [i for i in range(800, 1000)]
    ...
    print("my loss", myloss(torch.from_numpy(test_data).reshape(test_data.shape[0], -1) / 100.0,
                            torch.from_numpy(test_maps).reshape(test_data.shape[0], -1) / 100.0).item() / test_data.shape[0])
